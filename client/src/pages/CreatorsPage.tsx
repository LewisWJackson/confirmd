...
function CreatorCard({ creator, onClick }: { creator: any; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="group cursor-pointer rounded-2xl border border-slate-200 bg-white p-6 hover:shadow-[0_6px_30px_rgba(0,0,0,0.06)] transition-all duration-500 hover:-translate-y-1"
    >
      <div className="flex items-start gap-4 mb-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-base font-black text-slate-900 tracking-tight truncate group-hover:text-cyan-600 transition-colors">
            {creator.channelName}
          </h3>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        ...
        <div className="text-center p-2 bg-slate-50 rounded-xl">
          <div className="text-lg font-black text-slate-900">{creator.totalClaims ?? 0}</div>
          <div className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Claims</div>
        </div>
        <div className="text-center p-2 bg-slate-50 rounded-xl">
          <div className="text-lg font-black text-slate-900">{creator.reliabilityScore ?? "—"}</div>
          <div className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Trust</div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-slate-100">
        ...
        <span className="text-[10px] font-black text-cyan-600 uppercase tracking-widest group-hover:text-cyan-700 transition-colors">
          View Profile →
        </span>
      </div>
    </div>
    ...
    return (
      <div className="animate-in fade-in duration-1000 relative z-10">
        <section className="max-w-7xl mx-auto px-6 md:px-12 pt-16 pb-8">
          <span className="text-[10px] font-black tracking-[0.5em] text-cyan-600 uppercase">Creator Intelligence</span>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 pt-16 pb-8">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          ...
          <button
            ...
            className="self-start md:self-auto px-6 py-3 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-cyan-600 transition-all shadow-lg"
          >
            View Leaderboard
          </button>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 pb-8">
        <div className="flex flex-col sm:flex-row gap-4">
          <input
            ...
            className="bg-slate-50 border border-slate-200 rounded-xl px-5 py-2.5 text-xs font-medium focus:ring-1 focus:ring-cyan-500/50 outline-none w-full sm:w-64 transition-all text-slate-900 placeholder:text-slate-400"
          />
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 pb-24">
        {isLoading ? (
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-16 text-center">
            ...
            <h3 className="text-xl font-black text-slate-900 tracking-tight">No creators found</h3>
